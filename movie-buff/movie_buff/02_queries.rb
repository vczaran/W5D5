...
def cast_list(title)
  ...
  Actor.select(:id, :name).joins(:movies).where('movies.title IN (?)', title).order(:ord)
...
def most_supportive
  ...
  Actor.select(:id, :name, 'COUNT(*) AS supporting_roles').joins(:castings).where('ord != 1').group(:actor_id).order('supporting_roles DESC').limit(2)
